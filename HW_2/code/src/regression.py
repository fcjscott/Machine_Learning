...
class PolynomialRegression() :
    ...
    def generate_polynomial_features(self, X) :
        ...
        pow_array = np.arange(2, m+1)

        if m != 1:
            for i in pow_array:
                phi = np.column_stack((phi, phi[:, 1]**i))
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            self.coef_ -= 2 * eta * X.T.dot(X.dot(self.coef_) - y)
            print(self.coef_)
    ...
    def plot_regression(self, Xt, yt, xmin=0, xmax=1, n=50, **kwargs) :
...
def main() :
    ...
    for m in mrange:
        ...
        train_error.append(model.rms_error(X_train, y_train))
        test_error.append(model.rms_error(X_test, y_test))
        if m == 5:
            model.plot_regression(Xt = X_train, yt = y_train)


    plt.plot(mrange, train_error,'r-', mrange, test_error,'b-')
